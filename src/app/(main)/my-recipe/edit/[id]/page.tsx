...
export default function EditRecipePage() {
  ...
  const [isStoreOnlyEdit, setIsStoreOnlyEdit] = useState(false);
  const [storeWriteMode, setStoreWriteMode] = useState(false);
  const [originalBaseRecipe, setOriginalBaseRecipe] = useState<SystemRecipe | null>(null);
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        const [authRes, recipeRes] = await Promise.all([
          fetch('/api/auth/me'),
        ...
        if (!recipeRes.ok) throw new Error("โหลดข้อมูลสูตรอาหารล้มเหลว");
        const json = await recipeRes.json();
        if (json.error) throw new Error(json.error);
        const r = json.data;

        const isRecipeOwner = currentUser && currentUser.id === r.userId;
        const isStorePostOwner =
          currentUser &&
        ...
        if (isSetEdit && myStorePost) {
          ...
          setSellingPrice(sp.sellingPrice?.toString() || "");
          setShopDescription(sp.storeDescription || "");
          setShopLocation(sp.storeLocation || "");
          setContactInfo(sp.contactInfo || "");
          setStoreVisibility((sp.visibility as "public" | "protected" | "private") || "public");
          
          if (sp.setIngredients && Array.isArray(sp.setIngredients)) {
            setSetIngredientsList(sp.setIngredients.map((si: any, idx: number) => ({
              id: idx + 1,
              name: si.name || "",
          ...
          if (sp.images && sp.images.length > 0) {
            setShopIngredientImages(sp.images.map((img: any) => ({ previewUrl: img.imageUrl })));
          }

          if (sp.videos && sp.videos.length > 0) {
            setShopIngredientVideo({ previewUrl: sp.videos[0].videoUrl });
          }
        } else {
          setPostAs("user");
        }
        ...
        if (r.referenceRecipeId) {
          ...
          setPickedRecipe({
            id: r.referenceRecipeId,
            title: r.referenceRecipe?.recipeName || "สูตรอ้างอิง (ID: " + r.referenceRecipeId + ")",
            ownerName: r.referenceRecipe?.user?.username || "ระบบ",
            matchTags: [],
            ingredients: [],
        ...
        if (isSetEdit) {
          const baseRecipe: SystemRecipe = {
            ...
            ownerName: r.user?.username || "ผู้ไม่ประสงค์ออกนาม",
            matchTags: [],
            ingredients: [],
            instructions: r.instructions || "",
          };
          setRecipeSourceMode("system");
          setOriginalBaseRecipe(baseRecipe);
          setPickedRecipe(baseRecipe);
        }

        if (r.images && r.images.length > 0) {
          setCoverImages([
            ...r.images.map((img: any) => ({ previewUrl: img.imageUrl }))
          ]);
        }
  ...
  const validateForm = (isDraft: boolean): boolean => {
    ...
    // Store-only mode: only validate store post fields
    if (isStoreOnlyEdit) {
      const storeMissing: string[] = [];
      if (!shopName.trim()) storeMissing.push("shopName");
      if (!sellingPrice || parseFloat(sellingPrice) <= 0) storeMissing.push("sellingPrice");
      if (!shopDescription.trim()) storeMissing.push("shopDescription");
      if (!shopLocation.trim()) storeMissing.push("shopLocation");
      if (!contactInfo.trim()) storeMissing.push("contactInfo");
      ...
      if (storeMissing.length > 0) {
        setMissingFields(storeMissing);
        setSubmitError("กรุณากรอกข้อมูลร้านค้าให้ครบถ้วนทุกช่อง (รวมถึงรูปภาพอย่างน้อย 1 รูป)");
        const firstMissingId = storeMissing[0];
      ...
      // Store-only "write new recipe" mode also requires the recipe content fields
      if (storeWriteMode) {
        const recipeMissing: string[] = [];
        if (!title.trim()) recipeMissing.push("title");
        if (!isDraft && !description.trim()) recipeMissing.push("description");
        ...
        if (recipeMissing.length > 0) {
          setMissingFields(recipeMissing);
          setSubmitError("กรุณากรอกข้อมูลสูตรให้ครบถ้วนทุกช่อง (รวมถึงรูปภาพอย่างน้อย 1 รูป)");
          const firstMissingId = recipeMissing[0];
          const element = document.getElementById(`form-field-${firstMissingId}`) || document.getElementById("publish-recipe-btn");
          if (element) {
            element.scrollIntoView({ behavior: "smooth", block: "center" });
          }
  ...
  const submitForm = async (isDraft: boolean = false) => {
    ...
    try {
      // อัปโหลดไฟล์พร้อมกัน; ไฟล์ที่อัปโหลดไว้แล้วใช้ URL เดิม
      ...
      // Recipe content is sent when the recipe owner edits, or a store-only
      // user writes a brand-new recipe of their own
      const needsRecipeContent = !isStoreOnlyEdit || storeWriteMode;
      const contentPayload: Record<string, unknown> = {};

      if (needsRecipeContent) {
        contentPayload.recipeName = title.trim();
        contentPayload.description = description.trim();
        contentPayload.instructions = instructions.trim();
        contentPayload.visibility = isDraft ? "draft" : visibility;
        contentPayload.ingredients = validIngredients.map(i => ({
          name: i.name.trim(),
          quantity: parseFloat(i.quantity) || 1,
          unit: i.unit.trim(),
          category: i.category || undefined
        }));
        contentPayload.equipmentItems = validEquipments.map(e => ({ name: e.name.trim() }));
        contentPayload.referenceRecipeId = pickedRecipe ? pickedRecipe.id : null;
        ...
        if (uploadedRecipeImages.length > 0) {
          contentPayload.featuredImageUrl = uploadedRecipeImages[0];
          contentPayload.images = uploadedRecipeImages;
        }
        ...
        if (uploadedRecipeVideos.length > 0) {
          contentPayload.videos = uploadedRecipeVideos;
        }
      }

      // Store-only "write new recipe" mode: create the new recipe first,
      // then link the existing store post to it (via the PATCH below)
      let newRecipeId: string | null = null;
      if (isStoreOnlyEdit && storeWriteMode) {
        ...
        if (!createResponse.ok) {
          ...
          try {
            ...
            data = { error: rawBody || `HTTP ${createResponse.status}` };
          }
          ...
          const errorMessage =
            typeof apiError === "string"
              ...
              : apiError
                ...
                : rawBody || `เกิดข้อผิดพลาด (HTTP ${createResponse.status})`;
          setSubmitError(errorMessage || "เกิดข้อผิดพลาดในการสร้างสูตรใหม่ กรุณาลองใหม่อีกครั้ง");
          setIsSubmitting(false);
          return;
        }

        const createResult = await createResponse.json();
        newRecipeId = createResult.data?.id;
        if (!newRecipeId) {
          setSubmitError("ไม่สามารถสร้างสูตรใหม่ได้ กรุณาลองใหม่อีกครั้ง");
          setIsSubmitting(false);
          return;
        }
      }

      // Build the PATCH payload (always targets the OLD recipe so the existing
      // store post is found & relinked to the chosen/new recipe)
      const payload: Record<string, unknown> = {};
      if (!isStoreOnlyEdit) {
        Object.assign(payload, contentPayload);
      }
      ...
      if (postAs === "store") {
        ...
        // to the brand-new recipe they just wrote
        if (isStoreOnlyEdit) {
          if (storeWriteMode && newRecipeId) {
            storePayload.recipeId = newRecipeId;
          } else if (!storeWriteMode && pickedRecipe?.id) {
            storePayload.recipeId = pickedRecipe.id;
          }
        }
  ...
  if (loading) {
    return (
      <div className={`min-h-screen bg-[#F5EFD7] pb-20 overflow-x-hidden`} style={{fontFamily: 'sans-serif'}}>
        <Navbar />
    ...
    <div className="min-h-screen bg-[#F5EFD7] font-sans pb-20 overflow-x-hidden relative z-0">
      ...
      <main className="w-[95%] max-w-[1200px] mx-auto px-4 relative z-10 mt-8">
        ...
        <div className="bg-white border border-[#71B254] rounded-sm p-8 md:p-10 shadow-sm relative z-10">
          ...
          {isStoreOnlyEdit && (
            <div className="mb-8 p-4 bg-amber-50 border border-amber-300 text-amber-800 rounded-md text-sm font-medium leading-relaxed">
              คุณเป็นเจ้าของเซ็ทอาหารของร้านค้า (สามารถแก้ไขข้อมูลร้านค้าได้เท่านั้น) — ข้อมูลสูตรอาหารต้นฉบับไม่สามารถแก้ไขได้
            </div>
          )}
...
<div className="pt-4" id="form-field-setIngredients">
          ...
          {isStoreOnlyEdit && (
            <div className="mb-10 pb-10 border-b border-[#71B254]">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-full bg-[#71B254] text-white flex items-center justify-center font-bold text-lg">2</div>
                <h2 className="text-2xl font-bold text-gray-800">สูตรอาหารต้นฉบับ</h2>
              </div>

              <div className="pl-11 flex flex-col gap-4">
                <p className="text-sm text-gray-600 leading-relaxed">
                  เซ็ทอาหารของคุณจะจัดขายตามสูตรต้นฉบับนี้ — เลือกจากสูตรอาหารต้นฉบับ หรือพิมพ์สูตรเองได้ (แต่ไม่สามารถแก้ไขเนื้อหาสูตรของผู้อื่น)
                </p>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setStoreWriteMode(false);
                      setRecipeSourceMode("system");
                      setPickedRecipe(originalBaseRecipe);
                    }}
                    className={`flex-1 py-2.5 rounded-md font-bold text-sm border transition-colors ${
                      !storeWriteMode
                        ? "bg-[#71B254] text-white border-[#71B254]"
                ...
                {pickedRecipe ? (
                  <div className="bg-[#F4FAF1] border border-[#71B254] rounded-md p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="text-xs font-bold text-[#71B254] mb-1">สูตรที่ใช้อยู่</div>
                        <div className="font-bold text-gray-800 text-base">{pickedRecipe.title}</div>
                        <div className="text-xs text-gray-500 mt-1">โดย {pickedRecipe.ownerName}</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setPickedRecipe(null)}
                        className="px-3 py-1.5 border border-[#71B254] text-[#71B254] rounded-md text-xs font-bold hover:bg-[#F4FAF1] transition-colors shrink-0"
                      >
                        เปลี่ยนสูตร
                      </button>
                    </div>
                  ...
                  <>
                    <div className="bg-[#FBFAF3] border border-[#71B254] rounded-md p-5">
                      <label htmlFor="store-only-recipe-search" className="block text-gray-700 text-sm font-semibold mb-2">
                        ค้นหาสูตรอาหารต้นฉบับด้วยวัตถุดิบ (คั่นด้วยจุลภาค)
                      </label>
                      <input
                        id="store-only-recipe-search"
                        type="text"
                      ...
                      />
                      <div className="flex flex-col gap-2 min-h-[100px] max-h-[260px] overflow-y-auto pr-2">
                        {isLoadingRecipes ? (
                          ...
                          filteredSystemRecipes.map((r) => (
                            <div key={r.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-md bg-white hover:border-[#71B254] transition-colors">
                              <div>
                    ...
                    <button
                      type="button"
                      onClick={() => setPickedRecipe(originalBaseRecipe)}
                      className="w-fit px-3 py-1.5 text-gray-500 hover:text-gray-700 text-xs font-semibold"
                    >
                      ยกเลิกการเปลี่ยนสูตร (ใช้สูตรเดิม)
                    </button>
                  </>
                )}
                </>
          ...
          {(!isStoreOnlyEdit || storeWriteMode) && (
          ...
          <div className="flex flex-col gap-4 pt-8 border-t border-gray-100 relative z-20">
            ...
            <div className={`grid grid-cols-1 ${isStoreOnlyEdit ? "" : "md:grid-cols-2"} gap-4`}>
              {!isStoreOnlyEdit && (
                <button 
                  type="button" 
                  id="save-draft-btn" 
