...
const mockSearchResults = [
  {
    ...
    likes: 22,
    rating: 3.0,
    initialFavorite: false, // สถานะเริ่มต้นยังไม่ได้กดใจ (ตามรูป)
  },
  {
    id: 2,
  ...
  },
  {
    ...
    likes: 65,
    rating: 2.5,
    initialFavorite: true, // ตัวนี้กดใจสีแดงไว้ (ตามรูป)
  },
];
...
function ResultsContent() {
  ...
  // จำลองดึงคำที่ค้นหาเพื่อเอามาตั้งเป็นหัวข้อ (ถ้าไม่มีให้ขึ้นว่า Salad เป็นค่าเริ่มต้น)
  const queryTitle = searchParams.get("query") || "Salad";
  
  // State จัดการระบบกดหัวใจของการ์ดแต่ละใบแยกกัน
  const [favorites, setFavorites] = useState<Record<number, boolean>>(
    mockSearchResults.reduce((acc, current) => ({ ...acc, [current.id]: current.initialFavorite }), {})
  );
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-sm p-8 md:p-12 shadow-sm">
      ...
      <div className="mb-8 flex items-baseline gap-2">
        ...
        <span className="text-2xl font-medium text-gray-400">({mockSearchResults.length * 4})</span> 
      </div>
            ...
            >
              ...
              <div className="flex flex-col items-end justify-between w-full md:w-32 shrink-0 py-1">
                ...
                <div className="flex flex-col items-end gap-3 w-full">
                  {/* ยอดกดไลก์หัวใจ (สลับสีตามรูปดีไซน์ต้นฉบับได้) */}
                  <div 
                ...
                {/* ปุ่มเปิดวาร์ปไปดูวิธีทำตัวเต็ม */}
                <Link 
...
export default function SearchResultsPage() {
  return (
    <div className="min-h-screen bg-[#F5EFD7] font-sans pb-20 overflow-x-hidden">
      {/* 1. ดึงแถบเมนูนำทางด้านบนอันเดิมมาสวมใช้ได้เลยทันที */}
      <Navbar />

      {/* 2. ส่วนเนื้อหาโครงสร้างการ์ดแนะนำสูตรอาหาร */}
      <main className="w-[95%] max-w-[1000px] mx-auto px-4 mt-8">
        <Suspense fallback={<div className="text-center py-20 font-bold text-[#71B254]">Analyzing ingredients...</div>}>
          <ResultsContent />
        </Suspense>
      </main>
    </div>
